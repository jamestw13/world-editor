class World {
  ...
  #generateTrees(count = 10) {
    ...
    while (tryCount < 100) {
      ...
      // check if tree is near building or road
      let keep = true;
      ...
      // check if tree is close enough to something
      if (keep) {
  ...
  draw(ctx, viewPoint) {
    ...
    const assets = [...this.trees, ...this.buildings];

    assets.sort((a, b) => b.base.distanceToPoint(viewPoint) - a.base.distanceToPoint(viewPoint));

    for (const asset of assets) {
      asset.draw(ctx, viewPoint);
    }
  }
}
